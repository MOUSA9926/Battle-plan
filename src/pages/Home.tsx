...
import { Shield, Clock, Target, Crown, Building2, Users, Swords, Zap, AlertTriangle, ArrowDown, Youtube, ListChecks, Music, Pause, Play, X, PawPrint, Loader2 } from "lucide-react";
  ...
  return (
    <div className="flex flex-col items-center relative z-[60] w-full mt-6 justify-center h-[55px]">
      <motion.form
        ref={containerRef}
        animate={{ 
          width: isInputState ? 280 : 180,
          backgroundColor: isInputState ? 'rgba(18, 19, 22, 0.95)' : 'rgba(34, 211, 238, 0.05)'
        }}
        transition={{ type: 'spring', stiffness: 400, damping: 30 }}
        onSubmit={handleLogin}
        onClick={() => { if (!isInputState) setState('inputting'); }}
        className={`relative flex items-center h-full rounded-full overflow-hidden border ${isInputState ? 'border-cyan-500/50 shadow-[0_0_20px_rgba(34,211,238,0.2)]' : 'border-cyan-500/30 shadow-[0_0_15px_rgba(34,211,238,0.1)] cursor-pointer hover:bg-cyan-500/10 hover:border-cyan-400/60 hover:shadow-[0_0_25px_rgba(34,211,238,0.2)]'} transition-colors duration-300`}
        dir="rtl"
      >
        {/* ICON ON THE RIGHT (Consistent) */}
        <div className="absolute right-0 top-0 bottom-0 w-14 flex items-center justify-center z-10 transition-colors pointer-events-none">
            {state === 'verifying' ? (
               ...
               <PawPrint className={`w-5 h-5 sm:w-6 sm:h-6 transition-all duration-300 ${isInputState ? (password.length > 0 ? 'text-cyan-400 drop-shadow-[0_0_8px_rgba(34,211,238,0.6)]' : 'text-gray-400') : 'text-cyan-300 drop-shadow-[0_0_5px_rgba(34,211,238,0.4)]'}`} />
            )}
        </div>

        {/* Text for initial state */}
        <div 
          className={`absolute inset-0 right-14 flex items-center font-bold text-cyan-50 tracking-wide pointer-events-none transition-opacity duration-300 ${!isInputState ? 'opacity-100' : 'opacity-0'}`}
        >
          <span className="text-sm sm:text-base whitespace-nowrap">تسجيل الدخول</span>
        </div>
        ...
        <input
          ...
          className={`w-full h-full bg-transparent text-white pr-14 pl-12 outline-none transition-opacity duration-300 text-right opacity-0 ${isInputState ? 'opacity-100 pointer-events-auto' : 'pointer-events-none'}`}
        />

        {/* Submit arrow on the left when typing */}
        <div 
          className={`absolute left-1 top-0 bottom-0 w-12 flex items-center justify-center z-10 transition-opacity duration-300 ${isInputState ? 'opacity-100' : 'opacity-0 pointer-events-none'}`}
        >
          <button
            type="submit"
        ...
        <div className="max-w-7xl mx-auto px-6 lg:px-8 text-center flex flex-col items-center h-full w-full">
          ...
          >
            ...
            <div className="flex flex-col items-center justify-center relative">
              ...
              <div className={`w-full transition-all duration-1000 ${!isAuthenticated ? 'blur-[8px] opacity-70 pointer-events-none select-none' : ''}`}>
              ...
              <div className="mt-20 sm:mt-28 relative flex flex-col items-center w-full">
                ...
                >
                  ...
                  <div className={`relative w-full h-[350px] sm:h-[450px] mt-6 bg-[#121316] rounded-2xl overflow-hidden border border-white/5 max-w-2xl mx-auto shadow-2xl transition-all duration-1000 transform-gpu ${!isAuthenticated ? 'blur-2xl opacity-10' : ''}`}>
                      <div className="absolute inset-0 opacity-10" style={{ backgroundImage: 'linear-gradient(#fff 1px, transparent 1px), linear-gradient(90deg, #fff 1px, transparent 1px)', backgroundSize: '20px 20px' }}></div>
